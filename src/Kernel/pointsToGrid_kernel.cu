...
void pointToGridTemplate(const GridData<InputCoordT, InputValueT>& in_data, const float voxelSize, GridData<OutputCoordT, OutputValueT>& out_data) {
    ...
    cudaCheck(cudaMemcpyAsync(d_coords, (OutputCoordT*)in_data.pCoords, npoints * sizeof(OutputCoordT), cudaMemcpyHostToDevice));

    // Generate a NanoVDB grid that contains the list of voxels on the device
    nanovdb::GridHandle<nanovdb::CudaDeviceBuffer> handle = nanovdb::cudaVoxelsToGrid<OutputValueT>(d_coords, npoints, voxelSize);
    ...
    cudaCheck(cudaMemcpyAsync(d_values, reinterpret_cast<const OutputValueT*>(in_data.pValues), npoints * sizeof(OutputValueT), cudaMemcpyHostToDevice));
...
extern "C" void pointToGridFloat(const OpenFloatGrid& in_data, const float voxelSize, NanoFloatGrid& out_data) {
	pointToGridTemplate<openvdb::Coord, float, nanovdb::Coord, float, nanovdb::FloatGrid, nanovdb::SetVoxel<float>>(in_data, voxelSize, out_data);
}

extern "C" void pointToGridVector(const OpenVectorGrid& in_data, const float voxelSize, NanoVectorGrid& out_data) {
	pointToGridTemplate<openvdb::Coord, openvdb::Vec3f, nanovdb::Coord, nanovdb::Vec3f, nanovdb::Vec3fGrid, nanovdb::SetVoxel<nanovdb::Vec3f>>(in_data, voxelSize, out_data);
}

...
class VdbPrimCIterator {
   ...
   protected:
	...
	VdbPrimCIterator(const GEO_Detail*, GA_Range::safedeletions, const GA_PrimitiveGroup* = nullptr,
...
/// iterator to iterate over all VDB grids belonging to a gdp and, optionally,
/// belonging to a particular group.
class VdbPrimIterator : public VdbPrimCIterator {
   public:
	/// @param gdp
	///     the geometry detail over which to iterate
	/// @param group
...
/// @note This operation clears the input grid's metadata.
GU_PrimVDB* replaceVdbPrimitive(GU_Detail& gdp, const GridPtr& grid, GEO_PrimVDB& src, const bool copyAttrs = true,
...
using ScalarGridTypes = openvdb::TypeList<openvdb::BoolGrid, openvdb::FloatGrid, openvdb::DoubleGrid,
                                          openvdb::Int32Grid, openvdb::Int64Grid>;

using NumericGridTypes =
    openvdb::TypeList<openvdb::FloatGrid, openvdb::DoubleGrid, openvdb::Int32Grid, openvdb::Int64Grid>;
...
inline bool GEOvdbApply(const GEO_PrimVDB& vdb, OpT& op) {
...
template <typename GridTypeListT, typename OpT>
inline bool GEOvdbApply(GEO_PrimVDB& vdb, OpT& op, bool makeUnique = true) {
	if (vdb.hasGrid()) {
...
template <typename GridTypeListT, typename OpT>
inline bool GridvdbApply(openvdb::GridBase::Ptr& vdb, OpT& op, const bool makeUnique) {
	if(vdb == nullptr) return false;

	if (makeUnique) {
		if (const auto treePtr = vdb->baseTreePtr(); treePtr.use_count() > 2) {
			vdb->apply<GridTypeListT>([](Grid& baseGrid) { baseGrid.setTree(baseGrid.constBaseTree().copy()); });
		}
	}

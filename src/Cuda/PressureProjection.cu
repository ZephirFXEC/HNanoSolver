...
extern "C" void ComputeDivergence(nanovdb::GridHandle<nanovdb::CudaDeviceBuffer>& vel_handle, NanoFloatGrid& out_div, const cudaStream_t& stream) {
	...
	cudaMemset(d_value, 0, out_div.size * sizeof(float));
	...
	lambdaKernel<<<numBlocks, numThreads, 0, stream>>>(out_div.size, [=] __device__(const size_t tid) {
		...
		nanovdb::Vec3f vel_xm1 = velAccessor.getValue(d_coord[tid] - nanovdb::Coord(1, 0, 0));
		nanovdb::Vec3f vel_xp1 = velAccessor.getValue(d_coord[tid] + nanovdb::Coord(1, 0, 0));
		nanovdb::Vec3f vel_ym1 = velAccessor.getValue(d_coord[tid] - nanovdb::Coord(0, 1, 0));
		nanovdb::Vec3f vel_yp1 = velAccessor.getValue(d_coord[tid] + nanovdb::Coord(0, 1, 0));
		nanovdb::Vec3f vel_zm1 = velAccessor.getValue(d_coord[tid] - nanovdb::Coord(0, 0, 1));
		nanovdb::Vec3f vel_zp1 = velAccessor.getValue(d_coord[tid] + nanovdb::Coord(0, 0, 1));

...
enum class PointType : uint32_t {
	Disable = 0,  // no point information e.g. when BuildT != Point
...
template <typename T>
fancy_ptr<T> make_fancy(const T *ptr = nullptr) {
	return fancy_ptr<T>(ptr);
}
...
GridHandle<BufferT> cudaPointsToGrid(const PtrT dWorldPoints, int pointCount, double voxelSize = 1.0, PointType type = PointType::Default,
                                     const BufferT &buffer = BufferT(), cudaStream_t stream = 0);

//================================================================================================

template <typename BuildT, typename PtrT, typename BufferT = CudaDeviceBuffer, typename AllocT = cub::CachingDeviceAllocator>
GridHandle<BufferT> cudaPointsToGrid(std::vector<std::tuple<const PtrT, size_t, double, PointType>> pointSet,
                                     const BufferT &buffer = BufferT(), cudaStream_t stream = 0);
...
/// @param voxelCount number of voxel in the list @c dGridVoxels
/// @param voxelSize Size of a voxel in world units used for the output grid
/// @param buffer Instance of the device buffer used for memory allocation
/// @return Returns a handle with the grid of type NanoGrid<BuildT>
template <typename BuildT, typename PtrT, typename BufferT = CudaDeviceBuffer, typename AllocT = cub::CachingDeviceAllocator>
GridHandle<BufferT> cudaVoxelsToGrid(const PtrT dGridVoxels, size_t voxelCount, double voxelSize = 1.0, cudaStream_t stream = 0,
                                     const BufferT &buffer = BufferT());

//================================================================================================

template <typename BuildT, typename PtrT, typename BufferT = CudaDeviceBuffer, typename AllocT = cub::CachingDeviceAllocator>
GridHandle<BufferT> cudaVoxelsToGrid(std::vector<std::tuple<const PtrT, size_t, double>> pointSet, const BufferT &buffer = BufferT(),
...
__hostdev__ inline static Vec3T voxelToWorld(const Vec3u8 &voxel, const Coord &ijk, const Map &map) {
	...
	if constexpr (is_same<Vec3T, Vec3d>::value) {
		...
		return map.applyMapF(
		    Vec3f(ijk[0] + decode * voxel[0] - 0.5f, ijk[1] + decode * voxel[1] - 0.5f, ijk[2] + decode * voxel[2] - 0.5f));
	}
}
...
class CudaPointsToGrid {
   public:
	struct Data {
		...
		template <typename Vec3T>
		__hostdev__ Vec3T &getPoint(int i) const {
			return *(PtrAdd<Vec3T>(d_bufferPtr, blind) + i);
		}
	};  // Data
	...
	/// @param stream optional CUDA stream (defaults to CUDA stream 0)
	explicit CudaPointsToGrid(const Map &map, cudaStream_t stream = nullptr)
	    : mStream(stream), mPointType(is_same<BuildT, Point>::value ? PointType::Default : PointType::Disable) {
		mData.map = map;
	...
	/// @param stream optional CUDA stream (defaults to CUDA stream 0)
	explicit CudaPointsToGrid(const double scale = 1.0, const Vec3d &trans = Vec3d(0.0), cudaStream_t stream = nullptr)
	...
	template <typename T = BuildT>
	typename enable_if<is_same<T, Point>::value>::type setPointType(PointType type) {
		mPointType = type;
	}
	...
	GridHandle<BufferT> getHandle(const PtrT points, size_t pointCount, const BufferT &buffer = BufferT());
	...
	template <typename T = BuildT>
	typename enable_if<is_same<T, Point>::value, uint32_t>::type maxPointsPerVoxel() const {
		return mMaxPointsPerVoxel;
	}
	template <typename T = BuildT>
	typename enable_if<is_same<T, Point>::value, uint32_t>::type maxPointsPerLeaf() const {
		return mMaxPointsPerLeaf;
	}
   ...
   private:
	...
	BufferT getBuffer(PtrT points, size_t pointCount, const BufferT &pool);
...
template <typename BuildT, typename AllocT = cub::CachingDeviceAllocator>
__global__ void fillValueIndexKernel(const size_t numItems, uint64_t *devValueIndex,
                                     typename CudaPointsToGrid<BuildT, AllocT>::Data *d_data) {
...
/// or 'else' block of a constexpr if statement.
template <typename BuildT, typename AllocT = cub::CachingDeviceAllocator>
__global__ void leafPrefixSumKernel(const size_t numItems, uint64_t *devValueIndexPrefix,
                                    typename CudaPointsToGrid<BuildT, AllocT>::Data *d_data) {
	const int tid = blockIdx.x * blockDim.x + threadIdx.x;
	if (tid >= numItems) return;
...
template <typename PtrT, typename BufferT>
inline GridHandle<BufferT> CudaPointsToGrid<BuildT, AllocT>::getHandle(const PtrT points, size_t pointCount, const BufferT &pool) {
...
void CudaPointsToGrid<BuildT, AllocT>::countNodes(const PtrT points, size_t pointCount) {
	...
	mData.d_indx =
	    mMemPool.template alloc<uint32_t>(pointCount, mStream);  // uint32_t can index 4.29 billion Coords, corresponding to 48 GB
	...
	cudaLambdaKernel<<<numBlocks(pointCount), mNumThreads, 0, mStream>>>(
	    pointCount,
	    [=] __device__(size_t tid, const Data *d_data, const PtrT points) {
	...
	if (mVerbose)
		mTimer.restart("DeviceRadixSort of " + std::to_string(pointCount) + " voxel keys in " + std::to_string(mData.nodeCount[2]) +
		               " tiles");
	...
	for (uint32_t id = 0, offset = 0; id < mData.nodeCount[2]; ++id) {
		...
		cudaLambdaKernel<<<numBlocks(count), mNumThreads, 0, mStream>>>(
		    count,
		    [=] __device__(size_t tid, const Data *d_data) {
			    ...
			    if constexpr (is_same<BuildT, Point>::value)
				    p = is_same<Vec3T, Vec3f>::value ? d_data->map.applyInverseMapF(p) : d_data->map.applyInverseMap(p);
			    d_keys[tid] = voxelKey(id, p.round());
		    },
		    mDeviceData);
		cudaCheckError();
		CALL_CUBS(DeviceRadixSort::SortPairs, d_keys + offset, mData.d_keys + offset, d_indx + offset, mData.d_indx + offset, count, 0,
		          36);  // 9+12+15=36
		offset += count;
	}
	...
	CALL_CUBS(DeviceSelect::Unique, ShiftRightIterator<12>(mData.d_leaf_keys), d_keys, d_node_count + 1,
	          mData.nodeCount[0]);  // count lower nodes
...
inline BufferT CudaPointsToGrid<BuildT, AllocT>::getBuffer(const PtrT, size_t pointCount, const BufferT &pool) {
	auto sizeofPoint = [&]() -> size_t {
		switch (mPointType) {
			...
			case PointType::Voxel16:
				return sizeof(Vec3u16);
			case PointType::Voxel8:
				return sizeof(Vec3u8);
			case PointType::Default:
				return pointer_traits<PtrT>::element_size;
			default:
				return size_t(0);  // PointType::Disable
		}
	};
	...
	mData.lower =
	    mData.upper + NanoUpper<BuildT>::memUsage() * mData.nodeCount[2];  // upper internal nodes ends and lower internal nodes begin
...
inline void CudaPointsToGrid<BuildT, AllocT>::processGridTreeRoot(const PtrT points, size_t pointCount) {
	using Vec3T = typename remove_const<typename pointer_traits<PtrT>::element_type>::type;
	cudaLambdaKernel<<<1, 1, 0, mStream>>>(
	    1,
	    [=] __device__(size_t, Data * d_data, PointType pointType) {
		    // process Root
...
inline void CudaPointsToGrid<BuildT, AllocT>::processUpperNodes() {
	cudaLambdaKernel<<<numBlocks(mData.nodeCount[2]), mNumThreads, 0, mStream>>>(
	    mData.nodeCount[2],
	    [=] __device__(size_t tid, Data * d_data) {
		    auto &root = d_data->getRoot();
	...
	cudaLambdaKernel<<<numBlocks(valueCount), mNumThreads, 0, mStream>>>(
	    valueCount,
	    [=] __device__(size_t tid, Data * d_data) {
...
inline void CudaPointsToGrid<BuildT, AllocT>::processLowerNodes() {
	cudaLambdaKernel<<<numBlocks(mData.nodeCount[1]), mNumThreads, 0, mStream>>>(
	    mData.nodeCount[1],
	    [=] __device__(size_t tid, Data * d_data) {
		    auto &root = d_data->getRoot();
...
inline void CudaPointsToGrid<BuildT, AllocT>::processLeafNodes(const PtrT points) {
	...
	cudaLambdaKernel<<<numBlocks(mData.nodeCount[0]), mNumThreads, 0, mStream>>>(
	    mData.nodeCount[0],
	    [=] __device__(size_t tid, Data * d_data) {
	...
	cudaLambdaKernel<<<numBlocks(mData.voxelCount), mNumThreads, 0, mStream>>>(
	    mData.voxelCount,
	    [=] __device__(size_t tid, Data * d_data) {
	...
	cudaLambdaKernel<<<numBlocks(denseVoxelCount), mNumThreads, 0, mStream>>>(
	    denseVoxelCount,
	    [=] __device__(size_t tid, Data * d_data) {
	...
	if constexpr (BuildTraits<BuildT>::is_onindex) {
		...
		kernels::fillValueIndexKernel<BuildT, AllocT>
		    <<<numBlocks(mData.nodeCount[0]), mNumThreads, 0, mStream>>>(mData.nodeCount[0], devValueIndex, mDeviceData);
		cudaCheckError();
		CALL_CUBS(DeviceScan::InclusiveSum, devValueIndex, devValueIndexPrefix, mData.nodeCount[0]);
		mMemPool.free(devValueIndex);
		kernels::leafPrefixSumKernel<BuildT, AllocT>
		    <<<numBlocks(mData.nodeCount[0]), mNumThreads, 0, mStream>>>(mData.nodeCount[0], devValueIndexPrefix, mDeviceData);
		cudaCheckError();
	...
	if constexpr (BuildTraits<BuildT>::is_indexmask) {
		...
		kernels::setMaskEqValMaskKernel<BuildT, AllocT>
		    <<<numBlocks(mData.nodeCount[0]), mNumThreads, 0, mStream>>>(mData.nodeCount[0], mDeviceData);
		cudaCheckError();
	}
	if (mVerbose == 2) mTimer.stop();
...
inline void CudaPointsToGrid<Point>::processPoints(const PtrT points, size_t pointCount) {
	switch (mPointType) {
		...
		case PointType::PointID:
			cudaLambdaKernel<<<numBlocks(pointCount), mNumThreads, 0, mStream>>>(
			    pointCount, [=] __device__(size_t tid, Data * d_data) { d_data->template getPoint<uint32_t>(tid) = d_data->d_indx[tid]; },
			    mDeviceData);
			cudaCheckError();
			break;
		case PointType::World64:
			cudaLambdaKernel<<<numBlocks(pointCount), mNumThreads, 0, mStream>>>(
			    pointCount,
			    [=] __device__(size_t tid, Data * d_data) { d_data->template getPoint<Vec3d>(tid) = points[d_data->d_indx[tid]]; },
			    mDeviceData);
			cudaCheckError();
			break;
		case PointType::World32:
			cudaLambdaKernel<<<numBlocks(pointCount), mNumThreads, 0, mStream>>>(
			    pointCount,
			    [=] __device__(size_t tid, Data * d_data) { d_data->template getPoint<Vec3f>(tid) = points[d_data->d_indx[tid]]; },
			    mDeviceData);
			cudaCheckError();
			break;
		case PointType::Grid64:
...
inline void CudaPointsToGrid<BuildT, AllocT>::processBBox() {
	...
	cudaLambdaKernel<<<numBlocks(mData.nodeCount[1]), mNumThreads, 0, mStream>>>(
	    mData.nodeCount[1], [=] __device__(size_t tid, Data * d_data) { d_data->getLower(tid).mBBox = CoordBBox(); }, mDeviceData);
	cudaCheckError();
	...
	cudaLambdaKernel<<<numBlocks(mData.nodeCount[2]), mNumThreads, 0, mStream>>>(
	    mData.nodeCount[2], [=] __device__(size_t tid, Data * d_data) { d_data->getUpper(tid).mBBox = CoordBBox(); }, mDeviceData);
	cudaCheckError();
	...
	cudaCheckError()
	    ...
	    cudaLambdaKernel<<<numBlocks(mData.nodeCount[2]), mNumThreads, 0, mStream>>>(
	        mData.nodeCount[2],
	        [=] __device__(size_t tid, Data * d_data) { d_data->getRoot().mBBox.expandAtomic(d_data->getUpper(tid).bbox()); }, mDeviceData);
	cudaCheckError();

	// update the world-bbox in the root node
	cudaLambdaKernel<<<1, 1, 0, mStream>>>(
	    1, [=] __device__(size_t, Data * d_data) { d_data->getGrid().mWorldBBox = d_data->getRoot().mBBox.transform(d_data->map); },
	    mDeviceData);
	cudaCheckError();
...
template <typename BuildT, typename PtrT, typename BufferT, typename AllocT>
GridHandle<BufferT> cudaPointsToGrid(std::vector<std::tuple<const PtrT, size_t, double, PointType>> vec, const BufferT &buffer,
                                     cudaStream_t stream) {
	std::vector<GridHandle<BufferT>> handles;
	for (auto &p : vec)
		handles.push_back(cudaPointsToGrid<BuildT, AllocT>(std::get<0>(p), std::get<1>(p), std::get<2>(p), std::get<3>(p), buffer, stream));
...
template <typename BuildT, typename PtrT, typename BufferT, typename AllocT>
GridHandle<BufferT> cudaVoxelsToGrid(std::vector<std::tuple<const PtrT, size_t, double>> vec, const BufferT &buffer, cudaStream_t stream) {
	std::vector<GridHandle<BufferT>> handles;
	for (auto &p : vec)

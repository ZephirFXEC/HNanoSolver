...
struct Voxel {
	...
	__hostdev__ Voxel(const uint32_t d, const float t, const float f, const nanovdb::Vec3f& v)
...
class BrickMap {
   public:
	...
	__device__ static void requestAllocation(uint32_t x, uint32_t y, uint32_t z, AllocationRequest* req, RequestCounter* counter);

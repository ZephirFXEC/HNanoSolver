...
struct ToolOp {
	ToolOp(bool t, openvdb::util::NullInterrupter& boss, const openvdb::BoolGrid* mask = nullptr)
	    : mMaskGrid(mask), mThreaded(t), mBoss(boss) {}
...
SOP_VdbSolver::SOP_VdbSolver(OP_Network* net, const char* name, OP_Operator* op) : SOP_Node(net, name, op) {}

SOP_VdbSolver::~SOP_VdbSolver() = default;

OP_ERROR SOP_VdbSolver::cookMySop(OP_Context& context) {
	// we must lock our inputs before we try to access their geometry, OP_AutoLockInputs will automatically unlock our
	// inputs when we return
	OP_AutoLockInputs inputs(this);
	if (inputs.lock(context) >= UT_ERROR_ABORT) return error();

	duplicateSource(0, context);

	openvdb_houdini::HoudiniInterrupter boss((std::string("Computing VDB grids").c_str()));
...
openvdb_houdini::GridPtr SOP_VdbSolver::ProcessFloatVDBGrid(GU_PrimVDB* vdbPrim,
	...
	if (ToolOp<openvdb::tools::MeanCurvature> op(true, boss.interrupter(), nullptr);
		openvdb_houdini::GridvdbApply<openvdb_houdini::NumericGridTypes>(vdbPtrBase, op, false)) {
		outGrid = op.mOutGrid;
		}
